  ...
  data() {
    return {
      ...
      logs: [
        ...
        {
          version: "v3.3.5",
          date: "2020-10-19T10:00:00+0800",
          changes: [
            "添加：自适应清晰度物品图标",
            "添加：网络状态指示器显示当前正加载的数据项列表",

            "修复：首页等使用了视差加载效果的界面可能出现没有内容的错误",
            "修复：在刷新数据时停留在使用了关卡选择器的页面可能造成的显示问题",
            "修复：物品数据速览卡片在表格内可能遮挡一部分物品图标的问题",

            "优化：关卡选择器优化头部样式，提高跳转效率、增加背景图与选关界面模拟动画",
            "优化：提交界面网络错误提示于用户关闭前将持续显示",
            "优化：于掉落汇报和按关卡浏览统计数据中的章节名称显示效果",
        ...
        },
        {
          ...
          changes: [
            ...
            "优化：Planner 配置分享（现在在分享时会自动包含关卡排除、设置项、以及压缩配置文件；同时支持导入此前所有使用的配置文件格式以及支持导入不完整、亦或格式混合的配置文件）",
