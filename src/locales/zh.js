const messages = {
  ...
  meta: {
    ...
    footer: {
      copyright: {
        title: "许可协议",
        content:
          "素材掉落统计数据由企鹅物流统计，采用知识共享 署名-非商业性使用 4.0 国际 许可协议进行许可。转载、公开或以任何形式复制、发行、再传播本站任何内容时，必须注明从企鹅物流数据统计转载，并提供版权标识、许可协议标识、免责标识和作品链接；且未经许可，不得将本站内容或由其衍生作品用于商业目的。",
      },
  ...
  stats: {
    ...
    headerDesc: {
      ...
      quantity:
        "在该作战中，所有提交到企鹅物流数据统计的样本内，该物品的出现次数",
      times: "在该作战中，所有提交到企鹅物流数据统计的样本提交次数",
      percentage: "期望掉落概率；计算方式为 (掉落数 / 样本数)",
      patternPercentage:
        "此掉落组合占所有掉落汇报的百分比；计算方式为 (掉落数 / 样本数)",
      apPPR:
        "根据当前的「百分比」项，计算出的在该作战获得「1x对应物品」所需的期望理智，即 (作战理智消耗 / (掉落数 / 样本数))",
      clearTime:
        "来源为 PRTS Wiki 中的「最短通关用时」；其中，PRTS Wiki 对此的计算方法为：由「作战开始」直至「最后出怪」期间，使用 1 倍速的最短所需游戏时间总长",
      itemPerTime:
        "根据当前的「百分比」项，计算出的在该作战获得「1x对应物品」所需的期望时间消耗，即 (作战最短通关用时 / (掉落数 / 样本数))",
      timeRange:
        "由于存在掉落物列表变动故引入的「统计区间」概念。在本表所展示的数据中，其将仅包含已标注时间段内所提交的样本统计数据。若欲查看其它统计区间内的样本统计情况，请使用「高级查询」",
    },
    filter: {
      title: "数据过滤",
    ...
    site: {
      ...
      generating: {
        title: "全站数据一览正在计算中...",
        subtitle:
          "神经网络信息：由于数据量庞大，全站数据一览需要大约 5 分钟进行计算。请稍后再来。",
        refresh: "刷新全站数据一览",
      },
    },
  },
  contribute: {
    ...
    newFolder: "锐意新建文件夹中",
    caption:
      "企鹅物流数据统计是一项非盈利的社区项目，我们的发展离不开大家的共同维护与支持。欢迎前往上方 GitHub 为我们点 Star、提出 Issues，我们当然同时也欢迎 Pull Requests。",
    contribute_0:
      "如果您有以下一项或多项经验，愿意为企鹅数据贡献自己的一份力量，请加QQ群：747099627",
  ...
  report: {
    alert: {
      ...
      contact: {
        ...
        activator: "联系作者",
        after:
          "（请附作战名称、实际掉落、若有掉落截图更佳），确认后我们会尽快予以修正。",
      },
      causes: {
        noDrop: "尚未选择任何掉落物品",
        limitation:
          "本次汇报与现有数据差距较大，继续提交可能导致此次汇报被判定为异常，无法计入全部统计数据中。",
      },
    },
    specialSideStory: {
      dialog: {
    ...
    recognition: {
      ...
      tips: {
        ...
        addImage: "点击此处加入图片",
        copyImage:
          "右键图片或长按图片可拷贝到剪贴板或保存图片；再次点击图片即可关闭",
        abnormal:
          "您提交识别的截图中有 {count} 张截图由于未成功通过质量监测，被标记为了「汇报排除」并将无法上传至本站",
      ...
      report: {
        ...
        partialFailed:
          "共计 {count} 次结算记录上传失败。请依照下方步骤排查可能出现的错误原因",
        partialFailedDesc: [
        ...
        caption:
          "感谢您的掉落汇报；由于缓存原因，您汇报的数据可能需要最多 20 分钟即可于全站数据集生效",
      },
      states: {
        pending: "正等待初始化...",
        initializing: "初始化中...",
      ...
      exceptions: {
        ...
        "DropAreaDropsQuantity::NotFound": {
          title: "存在未识别到数量的掉落",
          subtitle:
            "分辨率或质量较差的图片可能会导致此问题。请检查图片是否经过压缩处理",
        },
        ...
        "DropAreaDrops::Illegal": {
          title: "存在非法掉落",
          subtitle:
            "此关卡目前不应存在任何掉落，但图片中存在。这通常是由于非活动期间的活动掉落的导致的。",
        },
        "Result::False": {
          title: "非结算页面截图或内部错误",
          subtitle:
            "若这确实是一张结算页面截图，请检查您是否选择了正确的服务器",
        },
        "Stage::NotFound": {
          title: "未知关卡",
          subtitle:
            "这可能是关卡识别出现了错误，或此关卡不被收录（如剿灭作战、绝境作战等）",
        },
        "Stage::Illegal": {
          title: "非法关卡",
          subtitle: "此关卡目前不开放",
    ...
    dossolesHoliday: {
      ...
      content:
        "根据初步数据统计推测，标志物掉率计算事件可能不满足独立前提，即：在所有标志物被集齐前、其掉率可能会产生动态变化。因此，我们决定仅收集集齐所有标志物后的掉率数据",
    },
    usage: "左键增加，右键减少",
  },
  mirrors: {
  ...
  settings: {
    storageIssue:
      "本地缓存存储出现问题，应用可能不稳定；若持续出现此警告，请尝试通过「菜单 - 设置 - 重置本地数据与设置」清空缓存以修复",
    category: {
      appearance: "外观",
      data: "设置与数据",
      about: "关于",
    ...
    optimization: {
      lowData: {
        ...
        active: "低数据模式已启用",
        subtitle:
          "低数据模式将停止加载诸如作战列表背景图、全站背景立绘等不必要资源以减少网络数据的使用。",
      },
    },
    data: {
      server: "本地缓存数据包含服务器：",
      ...
      reset: {
        title: "重置本地数据与设置",
        subtitle:
          "将会删除所有本地数据与设置。删除后，所有本地设置均会重置至默认值并刷新页面，数据将需要重新加载。确定要继续吗？",
      },
    },
    push: {
      language: "消息语言",
  ...
  stage: {
    ...
    actions: {
      ...
      history: {
        name: "最近选择作战",
        empty: ["暂无最近选择作战", "访问任何作战页面后，记录即会于此处显示"],
        clear: "清空",
      },
      links: {
        "prts-wiki": "Wiki",
  ...
  query: {
    panel: {
      ...
      footer: {
        cache:
          "现在显示的查询结果数据为缓存数据。若需要获取最新数据，请再次「执行查询」。",
        disclaimer:
          "「高级查询」功能所产出的所有数据信息均受本站「数据许可协议」保护；本站不对「高级查询」功能所产出的所有数据信息做任何形式的承诺或背书。",
      },
    },
    disclaimer: {
      title: "使用前必读",
      content:
        "「高级查询」意在为有更复杂数据分析需求的用户提供以更高自由度使用企鹅物流数据统计内相关数据集之可能性。由于本功能的高自由度属性，在不了解统计学等相关学科的学术内容前，尝试分析此功能的查询结果可能会存在误导性。企鹅物流数据统计提醒各位对数据进行二次分析的刀客塔：请在分析时不要断章取义、故意使用本站数据引导舆论。高级查询功能的受众群体是熟悉统计学等学科的用户，开放后便于他们以更多维度分析数据集。我们欢迎对掉落数据感兴趣并进行客观与科学分析的刀客塔，但我们不欢迎带有个人情绪或既定立场的情况下、尝试错误地从本站数据中得出对任何实体不利结论的用户。",
    },
    title: {
      main: "主查询",
  ...
  matrixCategory: {
    ...
    all: {
      label: "全部",
      tooltip: "全部：此数据来源类别包含所有数据",
    },
    auto: {
      label: "自动",
      tooltip: "自动：自动数据来源指由第三方工具所自动汇报的掉落数据",
    },
    manual: {
      label: "手动",
      tooltip: "手动：手动数据来源指由用户使用企鹅物流数据统计的「网页或 App 端」中的「掉落汇报」或「截图识别」汇报的掉落数据",
    },
  },
  validator: {
    required: "必填",
  ...
  quotes: {
    doYouKnow: ["你知道吗？白面鸮头上的那一撮毛超级可爱！o(*≧▽≦)ツ"],
  },
  credits: {
    material: {
      title: "网站内容声明",
