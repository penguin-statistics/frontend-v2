const messages = {
  ...
  "meta": {
    ...
    "footer": {
      "copyright": {
        "title": "利用許諾",
        "content": "ドロップ統計データはクリエイティブ・コモンズ 表示 - 非営利 4.0 国際 ライセンスの下に提供されています。当サイトのいかなる内容の転載や公開を行う場合は、必ず内容の元となっているページまたはトップページのURLのリンクを行い、データ統計の転載を行ったことを明記しなければなりません。また許可無しに当サイトの内容または派生作品を商業目的で利用することを許可しません。"
      },
  ...
  "stats": {
    ...
    "filter": {
      "title": "フィルタ",
      "indicator": "フィルタ未適用 | {count} フィルタ適用中 | {count} フィルタ適用中",
  ...
  "contribute": {
    ...
    "caption": "Penguin Statisticは非営利コミュニティプロジェクトです。本プロジェクトの発展は、皆様のご助力が必要不可欠でございます。宜しければGitHubへ本レポジトリにスターをお付けください。イシューやプルリクエストも心よりお待ちしております。皆様のご支援とご協力を賜りますよう、よろしくお願い申し上げます。",
  ...
  "report": {
    ...
    "dossolesHoliday": {
      ...
      "content": "これまでのデータに基づいて、ラノベのドロップ事件は統計的な独立性を満たしていないと推測されます。全種類のラノベを獲得していないなら、そのドロップ率が動態的に変化し、本来のドロップ率とは異なる可能性があります。本来のドロップ率を正確に計算するため、全種類のラノベを獲得してからご報告ください。"
    },
  ...
  "settings": {
    ...
    "data": {
      ...
      "reset": {
        "title": "ローカルデータと設定初期化",
        "subtitle": "すべてのローカルデータと設定を削除します。その後ローカル設定はすべて初期化され、ページを刷新します。データを再読み込みする必要があります。本当に続けますか？"
      }
    }
  },
  "zone": {
  ...
  "planner": {
    "notices": {
      "autoExistence": "ArkPlannerは、選択したサーバーで開いていないステージを自動的に非表示にします"
    },
    "options": {
      "_name": "オプション",
    ...
    "craft": {
      ...
      "errors": {
        ...
        "notEnough": "加工には {need} 個の「{item}」が必要ですが、 {have} 個しか保有していません"
      },
      "plans": {
        "title": "加工プラン",
        "plan": "加工には {cost} 個の「{item}」を消耗して、 残り{remain} 個あります"
      },
      "success": " {sourceItems} を使って {amount} 個の {productItem} に加工しました"
    },
    "calculation": {
      "title": "計算結果",
      "tabs": {
